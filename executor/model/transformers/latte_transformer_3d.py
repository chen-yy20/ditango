...
from ditango.core.parallel_state import get_usp_group, get_osp_group
...
class LatteTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        #Original Input: torch.Size([2, 4, 16, 64, 64])
        ...
        # logger.info(f"{height=} {width=} {num_patches=}") # height=32 width=32 num_patches=1024
        ...
        ) # 在16个帧复制 encoder_hidden_states
        # hidden_states.shape=torch.Size([32, 1024, 1152]) encoder_hidden_states.shape=torch.Size([2, 13, 1152]) encoder_hidden_states_spatial.shape=torch.Size([32, 13, 1152])
        # ================= DoIT - usp split ===================
        if get_usp_group().world_size > 1:
            ...
            encoder_hidden_states_spatial = split_tensor_uneven(encoder_hidden_states_spatial, get_usp_group().world_size, dim=0, tensor_name="encoder")[get_usp_group().rank_in_group]
            if num_frame % get_usp_group().world_size == 0:
                num_frame = num_frame // get_usp_group().world_size 
            else:
                num_frame = num_frame // get_usp_group().world_size + 1 # padding
                
            if num_patches % get_usp_group().world_size == 0:
                num_patches = num_patches // get_usp_group().world_size 
            else:
                num_patches = num_patches // get_usp_group().world_size + 1 # padding
        ...
        splitted_batch_size = batch_size
        ...
        for i, (spatial_block, temp_block) in enumerate(
            ...
            if enable_temporal_attentions:
                # =========================== DoIT-osp switch ===========================
                # osp switch
                if get_osp_group().world_size > 1:
                    hidden_states = get_osp_group().all_to_all(hidden_states, scatter_dim=1, gather_dim=0)  
                # =========================== DoIT ===========================
                # (batch_size * num_frame) num_tokens hidden_size -> (batch_size * num_tokens) num_frame hidden_size
                hidden_states = hidden_states.reshape(
                    splitted_batch_size, -1, hidden_states.shape[-2], hidden_states.shape[-1]
                ).permute(0, 2, 1, 3)
                # logger.info(f"Mid - {hidden_states.shape=} {hidden_states[0][0][:5]=}")
                ...
                hidden_states = hidden_states.reshape(-1, hidden_states.shape[-2], hidden_states.shape[-1]).contiguous()
        ...
        # =========================== DoIT - usp gather ===========================
        if get_usp_group().world_size>1:
            hidden_states = get_usp_group().gather_uneven_tensors(hidden_states.contiguous(), dim=0)
            hidden_states = remove_padding_after_gather(hidden_states, tensor_name="hidden")
            # No need to remove_padding_after_gather for latte
        # =========================== DoIT ===========================
        # unpatchify
