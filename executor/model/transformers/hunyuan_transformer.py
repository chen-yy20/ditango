...
from hyvideo.modules.attenion import attention, get_cu_seqlens
from hyvideo.modules.posemb_layers import apply_rotary_emb
from hyvideo.modules.mlp_layers import MLP, MLPEmbedder, FinalLayer
...
from ditango.executor.model.attention_processor import HunYuan_RingProcessor
...
class MMDoubleStreamBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn_proc = HunYuan_RingProcessor(layer_id=layer_id)
    ...
    def forward(
        ...
        # )
        attn = self.attn_proc.ring_attention(
            q,
            k,
            v,
            seqlen_q=q.shape[1],
...
class MMSingleStreamBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn_proc = HunYuan_RingProcessor(
            layer_id= layer_id
        )
...
class HYVideoDiffusionTransformer(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.double_blocks = nn.ModuleList(
            [
                ...
                for layer in range(mm_double_blocks_depth)
            ]
        )
        ...
        self.single_blocks = nn.ModuleList(
            [
                MMSingleStreamBlock(
                    ...
                    qk_norm=qk_norm,
                    qk_norm_type=qk_norm_type,
                    layer_id=layer+mm_double_blocks_depth,
                    **factory_kwargs,
                )
                for layer in range(mm_single_blocks_depth)
            ]
        )
    ...
    def forward(
        ...
        if get_usp_group().world_size > 1:
            ...
            if freqs_cis is not None:
                cos_splits = split_tensor_uneven(freqs_cos, get_usp_group().world_size, dim=0)
                sin_splits = split_tensor_uneven(freqs_sin, get_usp_group().world_size, dim=0)
                freqs_cis = (cos_splits[get_usp_group().rank_in_group], sin_splits[get_usp_group().rank_in_group])
        ...
        # ditango Adapt 5
        if get_usp_group().world_size > 1:
            img = get_usp_group().all_gather(img, dim=1)
            txt = get_usp_group().all_gather(txt, dim=1)
            img = remove_padding_after_gather(img, "img")
            txt = remove_padding_after_gather(txt, "txt")
        # logger.debug(f"{img.shape=} {txt.shape=} {vec.shape=}")
