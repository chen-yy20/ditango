...
class StepVideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if get_cfg_group().world_size == 2: # DoIT
            prompt_embeds = torch.chunk(prompt_embeds, get_cfg_group().world_size, dim=0)[get_cfg_group().rank_in_group]
            prompt_embeds_2 = torch.chunk(prompt_embeds_2, get_cfg_group().world_size, dim=0)[get_cfg_group().rank_in_group]
            prompt_attention_mask = torch.chunk(prompt_attention_mask, get_cfg_group().world_size, dim=0)[get_cfg_group().rank_in_group]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(self.scheduler.timesteps):
                ...
                if i == 2:
                    enable_timing()
                    print(f"Enabled timing!", flush=True)
