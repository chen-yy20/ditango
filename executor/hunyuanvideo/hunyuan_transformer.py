    ...
    def forward(
        ...
        if get_usp_group().world_size > 1:
            ...
            if self.use_attention_mask:
                text_mask = split_tensor_uneven(text_mask, get_usp_group().world_size, dim=1)[get_usp_group().rank_in_group]
            if freqs_cos is not None:
                freqs_cos = split_tensor_uneven(freqs_cos, get_usp_group().world_size, dim=0)[get_usp_group().rank_in_group]
                freqs_sin = split_tensor_uneven(freqs_sin, get_usp_group().world_size, dim=0)[get_usp_group().rank_in_group]
                
        # img.shape=torch.Size([1, 59400, 3072]) txt.shape=torch.Size([1, 128, 3072]) freqs_cos.shape=torch.Size([59400, 128]) usp=2
