# Copied from Open-Sora-Plan/opensora/sample/pipeline_opensora.py
...
from ditango.executor.model.transformers import OpenSoraT2V_v1_3
from ditango.core.parallel_state import get_cfg_group
from ditango.core.feature_cache import exist_cache, get_cache
    ...
    def __call__(
        ...
        if self.do_classifier_free_guidance:
            ...
            if get_cfg_group().world_size > 1:
                assert get_cfg_group().world_size == 2, (f"classifier free guidance parallel worldsize must be 2, found {get_cfg_group().world_size}")
                prompt_embeds = negative_prompt_embeds if get_cfg_group().rank_in_group == 0 else prompt_embeds
                prompt_attention_mask = negative_prompt_attention_mask if get_cfg_group().rank_in_group == 0 else prompt_attention_mask
                if self.tokenizer_2 is not None:
                    prompt_embeds_2 = negative_prompt_embeds_2 if get_cfg_group().rank_in_group == 0 else prompt_embeds_2
                    prompt_attention_mask_2 = negative_prompt_attention_mask_2 if get_cfg_group().rank_in_group == 0 else prompt_attention_mask_2
            else:
            # ================================== DoIT =================================
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # ================================== DoIT =================================
                if exist_cache():
                    get_cache().update_timestep(i)
                if self.do_classifier_free_guidance and get_cfg_group().world_size == 1:
                ...
                # Before Transformer: latent_model_input.shape=torch.Size([2, 8, 24, 44, 80]) prompt_embeds.shape=torch.Size([2, 1, 512, 4096]) prompt_attention_mask.shape=torch.Size([2, 1, 512]) timestep.shape=torch.Size([2]) attention_mask.shape=torch.Size([2, 24, 44, 80])
                # logger.info(f"Before Transformer: {latent_model_input.shape=} {prompt_embeds.shape=} {prompt_attention_mask.shape=} {timestep.shape=} {attention_mask.shape=}")
        ...
        torch.cuda.synchronize()
        transformer_tok = time.time()
        logger.info(f"Transformer time: {transformer_tok - transformer_tik}")
        # ==================make sp=====================================
