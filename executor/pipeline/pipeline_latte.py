    ...
    def __call__(
        ...
        if do_classifier_free_guidance:
            if get_cfg_group().world_size > 1:
                assert get_cfg_group().world_size == 2, f"cfg parallel size should be 2, found {get_cfg_group().world_size}"
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # ================= DoIT ===================
                latent_model_input = torch.cat([latents] * 2) if (do_classifier_free_guidance and get_cfg_group().world_size == 1) else latents
                # ================= DoIT ===================
