...
from ditango.executor.model.transformers import CogVideoXTransformer3DModel
...
class CogVideoXPipeline(DiffusionPipeline):
  ...
  def check_inputs(
    ...
    if prompt_embeds is not None and negative_prompt_embeds is not None:
      if prompt_embeds.shape != negative_prompt_embeds.shape:
        raise ValueError("`prompt_embeds` and `negative_prompt_embeds` must have the same shape when passed directly, but"
  ...
  def __call__(
    ...
    if num_frames > 49:
      import warnings
      warnings.warn(
          "The number of frames is greater than 49, which may cause issues due to static positional embeddings. "
          "This limitation will be removed in future updates.",
          UserWarning
      )
    ...
    # split latents and prompt embs
    if do_classifier_free_guidance and get_cfg_group().world_size > 1:
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
      ...
      # import toolbox
      # prof = toolbox.timer.TorchProfilerContext(
      #   log_name="cog",
      ...
      for i, t in enumerate(timesteps):
        ...
        # perform guidance
        if use_dynamic_cfg:
          self._guidance_scale = 1 + guidance_scale * ((1 - math.cos(math.pi * ((num_inference_steps - t.item()) / num_inference_steps)**5.0)) / 2)
